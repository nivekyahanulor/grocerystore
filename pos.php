								...
								<tbody>
									...
									?>
										<tr class="item-row">
											...
											<!-- <td>
												<input type="hidden" min="1" step="any" name="uom[]" value="<?php echo $row['uom'] ?>" class="text-right">
												<p class="text-right"><?php echo $row['uom'] ?></p>
											</td> -->
											<td>
					...
					<div class="modal fade" id="pay_modal" role='dialog'>
					    <div class="modal-dialog modal-md" role="document">
					      ...
					      <div class="modal-footer">
					        <button type="button" disabled class="btn btn-primary" id='submit' onclick="$('#manage-sales').submit()">Pay</button>
...
<div id="tr_clone">
	...
	<tr class="item-row">
		...
		<!-- <td>
		<input type="hidden" min="1" step="any" name="uom[]" value="" class="text-right" readonly="">
			<p class="uom text-right">0</p>
		</td> -->
		<td>
...
<script>
			...
			$.ajax({
				...
				success:function(resp){
					...
					if(resp.available >= qty){
						...
						tr.find('[name="uom[]"]').val(uom)
						tr.find('[name="qty[]"]').val(qty)
						tr.find('[name="price[]"]').val("₱ " +  resp.price)
						var amount = parseFloat(price) * parseFloat(qty);
						tr.find('.amount').html(parseFloat(amount).toLocaleString('en-US',{style:'currency',maximumFractionDigits:2,minimumFractionDigits:2, currency:"PHP"}))
						...
						alert_toast("The product you selected only has <b>"+resp.available+"</b> left in the inventory. Please RESTOCK as soon as possible!",'danger')
	...
	function calculate_total(){
		...
		var amount = parseFloat(_this.find('[name="qty[]"]').val()) * parseFloat(_this.find('[name="price[]"]').val().replace("₱ ", ""));
	...
	$('[name="amount_tendered"]').keyup(function(){
		...
		if(tendered < tamount){
			document.getElementById("submit").disabled = true;
			$('[name="change"]').val(0)
			alert_toast("Insufficient amount tendered!",'danger');
			end_load();
			return false;
		} else{
